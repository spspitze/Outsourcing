...
# Total jobs matched
m_obs <- NROW(matched)
m_outsourced <- table(matched$outsourced)[["1"]]
...
oj_info <- table(!is.na(on_jobs$looped))[["TRUE"]]
oj_info_missed <- table(!is.na(on_jobs_miss$looped))[["TRUE"]] - looped_m_q_table[["8"]]
oj_outsourced <- table(on_jobs$outsourced)[["1"]]
oj_outsourced_missed <- table(on_jobs_miss$outsourced)[["1"]] - outsourced_m_q_table[["8"]]
...
labels <- c("In Employer History Roster and Employer Suplement",
...
# Match to the main dataset a variable that indicates above average outsourcing 
# in occupation
out_ind <- matched_jobs %>% 
  select(case_id, ind, outsourced) %>% 
  group_by(ind, outsourced) %>% 
  summarise(number = n()) %>% 
  spread(outsourced, number) %>% 
  rename(not = `0`, outsourced = `1`) %>% 
  drop_na(outsourced, ind) %>% 
  ungroup() %>% 
  mutate(per_out = round(outsourced / (outsourced + not) * 100, 0))

# For occ, also look if workers have weekly wages above or below occ average wage by gender
out_occ <- matched_jobs %>% 
  group_by(female, occ) %>% 
  mutate(
    wage_above_occ = log_real_wkly_wage > mean(log_real_wkly_wage, na.rm = T)
  ) %>% 
  select(case_id, occ, outsourced, wage_above_occ) %>% 
  group_by(occ, outsourced) %>% 
  summarise(
    number = n(),
    above = mean(wage_above_occ, na.rm = T)
  ) %>% 
  gather(variable, value, -(occ:outsourced)) %>% 
  unite(temp, outsourced, variable) %>% 
  spread(temp, value) %>% 
  rename(not = `0_number`, outsourced = `1_number`,
         not_per_above = `0_above`, outsourced_per_above = `1_above`) %>% 
...
# What percent of all jobs are outsourced? Save this number to use below
per_out_matched <- mean(matched_jobs$outsourced) * 100
  
# Track occupations with outsourcing rate above average 
occ_above <- out_occ %>% 
  mutate(occ_out_above = 1 * (per_out > per_out_matched)) %>%
  rename(occ_per_out = per_out) %>% 
  select(occ, occ_out_above, occ_per_out)

# Match this with main datasets
matched <- left_join(matched, occ_above, by = "occ") %>% 
  mutate(
    occ_out_above = ifelse(!is.na(occ_out_above), occ_out_above, 0),
    occ_per_out = ifelse(!is.na(occ_per_out), occ_per_out, 0)) %>% 
  group_by(case_id) %>% 
  mutate(ever_out_occ_above = max(occ_out_above)) %>% 
  ungroup()

matched_jobs <- left_join(matched_jobs, occ_above, by = "occ") %>% 
  mutate(
    occ_out_above = ifelse(!is.na(occ_out_above), occ_out_above, 0),
    occ_per_out = ifelse(!is.na(occ_per_out), occ_per_out, 0)) %>% 
  group_by(case_id) %>% 
