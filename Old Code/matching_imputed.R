# Note: this is an earlier version of the code that needed to impute matches
# between emp_sup_clean and on_jobs_clean. The NLS released official matches,
# making this imputation no longer necessary. I keep this code to compare 
# new results to old ones. Results are saved in 
# Cleaned Data/Previous Version (Imputed Match)

# This file takes emp_hist_rost_clean, emp_sup_clean, on_jobs_clean, demographics_clean,
...
clean_save_folder <- "../Cleaned Data/Previous Version (Imputed Match)/"
table_folder <- "../Tables/Previous Version (Imputed Match)/"
figure_folder <- "../Figures/Previous Version (Imputed Match)/NLSY 79 Matching/"
d_table_folder <- "../Drafts/Draft Tables/Previous Version (Imputed Match)/"
s_table_folder <- "../Slides/Slide Tables/Previous Version (Imputed Match)/"
...
rm("matches", "match_rhs", "on_jobs_miss", "ever_out_count", "match_list",
   "on_jobs", "ehr_es", "emp_sup", "hist_rost", "dem_gender", "ever_out_count",
...
# Save datasets
fwrite(matched, str_c(clean_save_folder, "matched.csv"), row.names = FALSE)
fwrite(matched_jobs, str_c(clean_save_folder, "matched_jobs.csv"), row.names = FALSE)
fwrite(timeline, str_c(clean_save_folder, "matched_timeline.csv"), row.names = FALSE)
fwrite(timeline_r, str_c(clean_save_folder, "matched_timeline_robust.csv"),
       row.names = FALSE)