...
# Also combine year and month job start and stop data into dates
# Do this by taking year * 12 + month
for (year in seq(2002, 2016, by = 2)){
  ...
  for (job in seq(1, 9)){
    ...
    for (var in date_vars){
      if (check(str_c(q, var, year, ".0", job, m), new_data)){
        new_data[[str_c(q, var, year, ".0", job)]] <- 
          (new_data[[str_c(q, var, year, ".0", job, y)]] * 12       
           + new_data[[str_c(q, var, year, ".0", job, m)]])
      }
    }
  }
  ...
  for (job in seq(10, 65)){
    ...
    for (var in date_vars){
      if (check(str_c(q, var, year, ".", job, m), new_data)){
        new_data[[str_c(q, var, year, ".", job)]] <- 
          (new_data[[str_c(q, var, year, ".", job, y)]] * 12       
           + new_data[[str_c(q, var, year, ".", job, m)]])
      }
    }
  }
}
...
long <- new_data %>% 
  ...
  rename(
    ...
    month_start_job = EMPLOYERS_ALL_STADATE,
    month_end_job = EMPLOYERS_ALL_STOPDATE,
    week_start_job = EMPLOYERS_ALL_STARTWEEK,
    week_end_job = EMPLOYERS_ALL_STOPWEEK,
...
long_clean <- long %>% 
  ...
  mutate(job_year = (month_end_job - 1) %/% 12,
  ...
  mutate(
    ...
    month_start_job = ifelse(same_job_lag & (month_start_job <= month_end_job_lag) %in% T,
                            NA, month_start_job),
    month_end_job = ifelse(same_job_lead & (month_end_job >= month_start_job_lead) %in% T,
                          NA, month_end_job)
  ) %>% 
...
long_transtitions <- long_clean %>% 
  ...
  mutate(week_end_job_lag = lag(week_end_job),
         ...
         weeks_between_jobs = week_start_job - week_end_job_lag,
         # Set more than 6 months before or 2 years after to NA for 
         # length and previous/next job
         previous_job = ifelse(weeks_between_jobs < -26, NA, previous_job),
         next_job = ifelse(weeks_between_jobs > 104, NA, next_job),
         weeks_between_jobs = ifelse(
           weeks_between_jobs < -26 | weeks_between_jobs > 104, NA, 
           weeks_between_jobs)
         ) %>% 
