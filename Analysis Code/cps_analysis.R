...
cps <- cps |>
  ...
  mutate(
    ...
    # Keep values where hourwage exists and >= 3, 
    # earnweek exists and >= 60
    # and uhrswork1 exists and >0 <= 80
    hrly_keep = (hourwage >= 3 & hourwage != 999.99 
                 & !is.na(hourwage)),
    wkly_keep = (earnweek >= 60 & earnweek != 9999.99 
                 & !is.na(earnweek)),
    hour_keep = (uhrswork1 > 0 & uhrswork1 <= 80 
                 & !is.na(uhrswork1)),
    # Make wages real and take logs
    log_real_hrly_wage = 
      ifelse(hrly_keep, log(hourwage / cpi * base_cpi), NA),
    log_real_wkly_wage = 
      ifelse(wkly_keep, log(earnweek / cpi * base_cpi), NA),
...
from 2001-2016. High outsourcing occupations are occupations 
...
percent Black, Hispanic, and union member, average age, and 
...
Each regression contains controls for percent Black, Hispanic,
...
comp[[1]] <- filter(comp[[1]], !is.na(ho_occ))
...
saves <- c("", "NLSY Cohort ")
descriptions <- c("for all male", 
                  "for NLSY 79 cohort (born between 1957-1964) male")
labels <- c("", "_nlsy")
...
for (j in 1:3) {
  ...
  vs not from January 2001 - October 2016. Statstics are weighted 
...
label <- "ho_occ_cps"
note <- "
Summary statistics for male workers in the NLSY and CPS.
NLSY data takes job level aggregates for jobs held between
January 2001 - October 2016. CPS data comes from 
the monthly January 2001 - October 2016 surveys
for all employed men age 18-65 and for those born between 
1957-1964 (NLSY cohort). Workers are divided by if they work
in a high outsourcing (HO) occupation (all occupations with 
outsourcing more than 4.34\\% in the NLSY). Statstics are 
...
rm(data, data_pbs, data_r, ho_ss, matched_jobs,
   ...
   occ_timeline_r_m, pbs_timeline_m, comp, cond)
...
for (i in seq_along(vars)) {
  ...
  ggsave(str_c(figure_folder, save_name, 
               " Overall Distribution by HO Occ.pdf"),
         height = height, width = width)
  ...
  ggsave(str_c(figure_folder, save_name, 
               " Yearly Distribution by HO Occ.pdf"),
         height = height, width = width)
  ...
  # 
  # temp <- cps |>
  #   filter(!is.na(!!var_sym), !is.na(age),
