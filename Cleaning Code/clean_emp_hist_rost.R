# This file takes raw Employer History Supplement data from 
# emp_hist_rost_raw (and some CPI Data from CPIAUSCL) and creates
# emp_hist_rost_clean
...
for (job_n in 1:65){
  ...
  for (year in seq(2002, 2016, by = 2)){
    ...
    if (check_exists(uid_old, new_data)) {
    ...
    for (date_n in 1:2) {
      for (var in date_vars) {
        ...
        if (check_exists(date_old, new_data)) {
          new_data <- 
            rename(new_data, !!date_new := !!date_old)
        }
      }
    }
  }
...
vary <- c("UID_", "STOPDATE_", "STADATE_", "STOPWEEK_", "STARTWEEK_",
          "TENURE_", "HOURSWEEK_", "IND_", "OCC_", "HRLY_WAGE_",
          "UNION_")

constant <- c("CASEID")

# Variables we keep
keep <- str_c("^(", str_c(str_c(q, vary), collapse = "|"),
              "|", constant, ")")
...
long <- new_data |> 
  ...
  filter_at(vars(-case_id, -job, -int_year, -emp_id),
            any_vars(!is.na(.))) |> 
  ...
  mutate(
    month_start_job = make_date(EMPLOYERS_ALL_STADATE_Y,
                                EMPLOYERS_ALL_STADATE_M, 1),
    month_end_job = make_date(EMPLOYERS_ALL_STOPDATE_Y,
                              EMPLOYERS_ALL_STOPDATE_M, 1),
    week_start_job = round_week(EMPLOYERS_ALL_STARTWEEK),
    week_end_job = round_week(EMPLOYERS_ALL_STOPWEEK)
  ) |> 
  # In hist_roster, jobs are reported retrospectively. 
  # To find real wages, I plan on using CPI from year last reported
  # working that job that survey (job_year). 
  ...
  mutate(
    # Create job_year
    job_year = ifelse(!is.na(month_end_job), year(month_end_job),
                      int_year),
...
long <- long |>
  ...
  mutate(
    ...
    # Create weekly wages by multiplying hours_week by hrly_wage 
    # (drop 0 hrs). Make them real (in 2016 $"s) and take logs
    ...
    occ_cat = 1 * (occ > 0) + 1 * (occ >= 3600) + 1 * (occ >= 4700) 
    + 1 * (occ >= 6000) + 1 * (occ >= 6200) + 1 * (occ >= 7700) 
    + 1 * (occ >= 9800),
    ind_cat = 1 * (ind > 0) + 1 * (ind >= 370) + 1 * (ind >= 690) 
    + 1 * (ind >= 770) + 1 * (ind >= 1070) + 1 * (ind >= 4000) 
    + 1 * (ind >= 4600) + 1 * (ind >= 6000) + 1 * (ind >= 6400) 
    + 1 * (ind >= 6800) + 1 * (ind >= 7200) + 1 * (ind >= 7800) 
    + 1 * (ind >= 8500) + 1 * (ind >= 8700) + 1 * (ind >= 9300) 
    + 1 * (ind >= 9600)
  ) |> 
  ...
  mutate(
    ...
    # Create union_fill which is -1 if union is NA 
...
# If respondent holds a job for multiple periods, it seems the 
# job_history  reports that date_job_end one period = date_job_begin 
# next period, which is not what we want. 
# (Because On Jobs has this missing)
...
long <- long |>
  ...
  mutate(
    ...
    month_start_job = as_date(ifelse(year_rank == 1, 
                                     month_start_job, NA)),
    month_end_job = as_date(ifelse(year_rank == max(year_rank),
                                   month_end_job, NA))
  ) |> 
  # Drop unneeded variables
  select(-job, -job_year, -hrly_wage, -wkly_wage, -cpi,
         -work_drop, -year_rank)
...
# Save the data
fwrite(long, str_c(clean_folder, "emp_hist_rost_clean.csv"),
       row.names = FALSE)