...
demographics_merge <- demographics |> 
  ...
  select(case_id:birth_year, hh_child:tot_child, 
         less_hs:educ_other) |> 
  select(-female)
...
timeline <- timeline[
  working == 0, c("emp_id", "outsourced", "tenure", 
...
timeline_week_conflict <- timeline_week_conflict[,c("max", "non_na") 
                                                 := NULL]
...
timeline <- timeline[
  !is.na(emp_id),
  w_tenure := max_tenure + time_length(week - week_end_job,
                                       unit = "week")
  ]
# If negative, assume weeks are correct
timeline <- timeline[
  w_tenure < 0, w_tenure := time_length(week - week_start_job,
                                        unit = "week")
  ]
...
timeline <- timeline[
  ,c("working_next", "working_prev") := shift(.SD, c(-1,1)), 
  by = case_id, .SDcols = "working"]
timeline <- timeline[
  ,c("emp_id_next", "emp_id_prev") := shift(.SD, c(-1,1)), 
...
timeline <- timeline[, c(
  "max", "non_na", "week_start_match", "week_end_match", 
  "working_next", "working_prev", "emp_id_next", "emp_id_prev")
  := NULL]
...
fwrite(timeline, str_c(clean_folder, "matched_timeline_women.csv"), 
       row.names = FALSE)