...
unmatched_obs <- ehr_es_obs - NROW(matched)
...
oj_missed <- NROW(on_jobs_miss)
oj_info <- sum(!is.na(on_jobs$looped))
oj_info_missed <- sum(!is.na(on_jobs_miss$looped)) 
oj_outsourced <- sum(on_jobs$outsourced, na.rm = T)
oj_outsourced_missed <- sum(on_jobs_miss$outsourced, na.rm = T) 
...
# Impute Total Compensation -----------------------------------------------

# Use benefits_clean to impute total compensation based
# on occupation, year, log_real_wkly_wage, health, and retirement
# benefits_clean provides yearly expected value of access to 
# health and retirement benefits as a percent of wages by 
# occupation category. Total compensation is log real weekly
# wages plus these percent additions if the worker has access.

# Get benefits data. 
benefits <- read_csv(str_c(clean_folder, "benefits_clean.csv"),
                     col_types = cols(
                       .default = col_double(),
                       occupation = col_character()
                     ))

# Data only goes back to 2008. For 2001 - 2007, assume 2008's values
benefit_impute <- benefits |>
  filter(year == 2008)

for (y in seq(2001, 2007)) {
...
# 1. Add occupation category (benefits uses SOC to group occupations,
# which allows for the same groups as the Census occupation codes.
# see https://usa.ipums.org/usa/volii/occtooccsoc18.shtml#1)
# If no occupation code, use overall average.
# 2. Merge in value of benefits by int_year and occupation
# 3. Impute log total compensation
...
matched <- matched |>
  ...
  left_join(
    benefits, 
    by = c("int_year" = "year", "occupation" = "occupation")) |>
  mutate(
    multiplier = 
      1 + health * health_share + retirement * retirement_share,
    log_real_tot_comp = log_real_wkly_wage + log(multiplier)
  ) |>
  select(-occupation, -multiplier, -health_share, -retirement_share)

# Create Matched Jobs -----------------------------------------------------

# Variables to use mean, mode, max, and min 
mean_vars <- c("hours_week", "tenure", "union", "log_real_hrly_wage",
               "log_real_wkly_wage", "log_real_tot_comp",
               "part_time", "childcare", 
...
rm("on_jobs", "ehr_es", "emp_sup", "hist_rost", "linking",
   "oj_info", "oj_info_missed", "oj_missed",
...
temp_match <- matched_jobs |> 
  select(case_id, emp_id, hours_week, part_time, occ, ind, pbs, 
         ...
         log_real_wkly_wage, log_real_tot_comp, self_emp:traditional, 
...
# # When using only surveys 2012 and before, use where traditional starts
...
num_worker_job_interview <- NROW(matched) 

# Worker-week Level
num_worker_week <- NROW(timeline) 
  
table <- "\\begin{tabular}{lSS}
\\toprule
Level & observations \\\\ \\midrule
"

table <- str_c(
  table,
  "Worker", format_n(num_worker$n), 
  " \\\\ \n",
  "Worker-Job", format_n(num_worker_job$n), " \\\\ \n",
  "Worker-Job-Interview", format_n(num_worker_job_interview), 
  " \\\\ \n",
  "Worker-Week", format_n(num_worker_week), " \\\\ \n"
)
...
write.xlsx(l, file=str_c(table_folder, "ind_occ.xlsx"), overwrite=TRUE)

# Clear some space
rm(outsourcing_ind, ind_names, l)
...
name <- "Most Commonly Outsourced Jobs for Workers without Bachelor's Degree"
label <- "out_occs_ls"
...
header <- make_header("", name, label)
d_header <- make_header("d", name, label, loc="h!")
s_header <- make_header("s")
...
name <- "Most Commonly Outsourced Jobs for Workers with Bachelor's Degree"
label <- "out_occs_hs"
note <- "The ten most common outsourced jobs for workers
...
note <- "Outsourcing prevalence among occupations and workers.
...
fwrite(matched_jobs, str_c(clean_folder, "matched_jobs.csv"),
       row.names = FALSE)
fwrite(timeline, str_c(clean_folder, "matched_timeline.csv"),
       row.names = FALSE)
fwrite(timeline_r, str_c(clean_folder, "matched_timeline_robust.csv"), 
       row.names = FALSE)